...
def get_benchmark():
    ...
    if benchmark_name is not None:
        with open('./system_sim_state_3D/benchmark_name.txt', 'w') as f_out:
            f_out.write(benchmark_name + '\n')
    else:
        print("Warning: benchmark_name not found in config!")
    return benchmark_name

def Gradient(benchmark_name , P_k , core_num ,mem_num , matrix,Ac,Am,T_rest , power_per_bank):
    # benchmark_name : the name of benchmark
    ...
    # core_num : Core position index. The core of the index position will undergo optimization in the greedy algorithm
    # matrix: A vector containing all the active core frequencies in the previous epoch period
    # Ac : A GCB matrix show how does the i-th core power affect the maximum temperature of the i-th channel
    # Am : A GCB matrix show how does the i-th bank power affect the maximum temperature of the i-th channel
    # T_rest : Residual threshold temperature
    # power_per_bank : the predicted bank power
    e = 1e-10 # 
    if (benchmark_name == 'blackscholes') or (benchmark_name == 'swaption'):
        fun = lambda x : (-x[0]) / ( 0.014*x[0]*( 0.16*(x[1]*0.01)**2 + 0.12*(x[1]*0.01) + 0.62  )+ 0.725   ) ## 
    if (benchmark_name == 'bodytrack'):
        fun = lambda x : (-x[0]) / ( 1.9877 + 0.3098 * x[0] + -2.3148*x[1] + 0.0807*x[0]**2 + 0.1615*x[0]*x[1]  -0.4037*x[1]**2 ) ## 
    if (benchmark_name == 'streamcluster') or (benchmark_name == 'dedup'):
        fun = lambda x : (-x[0]) /(0.6145 + 0.3897*x[0] + 0.6145*x[1] - 0.0482*x[0]**2 - 0.0482*x[0]*x[1] - 0.6145*x[1]**2) ##
...
def combination(benchmark_name , Ac , Am , T_rest , power_per_bank,a):
    # benchmark_name : the name of benchmark
    # P_K : power vector of all cores at the previous step (assume current time is t, power budgeting/DVFS step is h, P_k is the power from time t-h to t, and we want to compute the power budget P for time t to t+h)
    # core_num : Core position index. The core of the index position will undergo optimization in the greedy algorithm
    # matrix: A vector containing all the active core frequencies in the previous epoch period
    ...
    # T_rest : Residual threshold temperature
    # power_per_bank : the predicted bank power
    # a : the utilization of cores


    freq_mem = [0.4,0.5 ,0.6,0.7,0.8,0.9,1.0] 
    #freq_mem = np.linspace(0.4, 1.0, 7)
    ...
    for j in range(0,len_mem):
        ...
        #print('the frequnecy core is ', freq_core)
        ...
        if (benchmark_name == 'blackscholes') or (benchmark_name == 'swaption'):
            CPI = ( 0.014*freq_core*( 0.16*(freq_mem[j]*0.01)**2 + 0.12*(freq_mem[j]*0.01) + 0.62  )+ 0.725   ) ## 
        if (benchmark_name == 'bodytrack'):
            CPI = ( 1.9877 + 0.3098 * freq_core + -2.3148*freq_mem[j] + 0.0807*freq_core**2 + 0.1615*freq_core*freq_mem[j]  -0.4037*freq_mem[j]**2 ) ## 
        if (benchmark_name == 'streamcluster') or (benchmark_name == 'dedup'):
            CPI = (0.6145 + 0.3897*freq_core + 0.6145*freq_mem[j] - 0.0482*freq_core**2 - 0.0482*freq_core*freq_mem[j] - 0.6145*freq_mem[j]**2) ##
        IPS =  freq_core / CPI
    ...
    #print(freq_c , freq_m)
    return freq_c , freq_m

def calculation(benchmark_name , res , freq_c ,freq_m):
    #res.x[1] = float(int(res.x[1] * 10) / 10  )
    ...
    res.x[1] = reference_values[closest_index]
    if (benchmark_name == 'blackscholes') or (benchmark_name == 'swaption'):
        CPI_1 = ( 0.014*res.x[0]*( 0.16*(res.x[1]*0.01)**2 + 0.12*(res.x[1]*0.01) + 0.62  )+ 0.725   ) ## 
        CPI_2 = ( 0.014*freq_c*( 0.16*(freq_m*0.01)**2 + 0.12*(freq_m*0.01) + 0.62  )+ 0.725   ) ## 
        IPS_1 = res.x[0] / CPI_1
        IPS_2 = freq_c / CPI_2
    if (benchmark_name == 'bodytrack'):
        CPI_1 = ( 1.9877 + 0.3098 * res.x[0] + -2.3148*res.x[1]  + 0.0807*res.x[0]**2 + 0.1615*res.x[0] *res.x[1]   -0.4037*res.x[1] **2 ) ## 
        CPI_2 = ( 1.9877 + 0.3098 * freq_c + -2.3148*freq_m + 0.0807*freq_c**2 + 0.1615*freq_c*freq_m  -0.4037*freq_m**2 ) ## 
        IPS_1 = res.x[0] / CPI_1
        IPS_2 = freq_c / CPI_2
    if (benchmark_name == 'streamcluster') or (benchmark_name == 'dedup'):
        CPI_1 = (0.6145 + 0.3897*res.x[0]  + 0.6145*res.x[1]  - 0.0482*res.x[0]**2 - 0.0482*res.x[0]*res.x[1]  - 0.6145*res.x[1] **2) ##
        CPI_2 = (0.6145 + 0.3897*freq_c + 0.6145*freq_m - 0.0482*freq_c**2 - 0.0482*freq_c*freq_m - 0.6145*freq_m**2) ##
        IPS_1 = res.x[0] / CPI_1
        IPS_2 = freq_c / CPI_2
    #print(res.x)
    #print(freq_c , freq_m)
    #print(IPS_1)
    #print(IPS_2)
    #error = float(abs((IPS_1 - IPS_2)/IPS_1))
    #print(error)
    error = abs(res.x[0] - freq_c)/res.x[0]
    #print(error)
...
def get_latest_CoreFreq():
    ############################################
    ##get the latest core frequency of core
...
def gdp_power(A, core_map, temp_max, temp_amb, P_s, P_k, T_c, gdp_mode):
    ...
    # number_access : the number of read and write accesses to a single bank in the last simulation epoch
    # benchmark_name : the benchmark name get from base config file
    # matrix : The frequency size of each active core in the last simulation epoch
    num_access = file_access()
    benchmark_name = get_benchmark()
    matrix = get_latest_CoreFreq()
    execution_time = 0
    err_init = 0
    ...
    #########################################
    # do not change here !
    # get the highest temperature layer in 3-D chip
    ...
    #print('time 1 is ,',execution_time1)
    print('the current temperature is ')
    print(T_c)
    print('the current power is ')
    print(P_k)
...
########################################################################################
    ...
    err_o = 0
    ## sorted_arr : the CoreMap using New mapping method 
    ...
    #########################
        ...
        res , a = Gradient(benchmark_name , P_k , core_num,mem_num , matrix,Ac,Am,T_rest,power_per_bank)

        start_time = time.time()
        freq_c , freq_m = combination(benchmark_name , Ac , Am , T_rest , power_per_bank,a);
        end_time = time.time()
        execution_time3 = end_time - start_time
        execution_time = execution_time2 + execution_time3 + execution_time

        #res , a = Gradient(benchmark_name , P_k , core_num,mem_num , matrix,Ac,Am,T_rest,power_per_bank)
        err = calculation(benchmark_name , res , freq_c ,freq_m)


        err_init = err + err_init
        #print('the max error is 'f"{err_o * 100:.1f}%")
