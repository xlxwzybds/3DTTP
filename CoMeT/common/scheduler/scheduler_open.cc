...
void SchedulerOpen::initMappingPolicy(String policyName) {
	...
	else if (policyName == "gdp")
	{
		...
		string filename = "./system_sim_state_3D/mapping.py ";
		string command = "python3 "+filename;
		system(command.c_str());

		std::string mapping_name;
		std::ifstream mappingfile("./system_sim_state_3D/mapping_name.txt");
		...
		if (mapping_name == "firstunused"){
			mappingPolicy = new MapFirstUnused(numberOfCores, preferredCoresOrder);
		}
		else if (mapping_name == "gdp"){
...
void SchedulerOpen::executeDramPolicy()
{
    ...
    //for (int i = 0; i < 16; ++i) { //         
    //         if (i==0 || i == 2 || i == 3 || i == 4 || i == 9 || i == 11 || i == 12 || i == 14) {
    //         //if (i == 2 ||  i == 9 || i == 11 || i == 0 ) {
    //     	    	//new_bank_modes[i] = 9;
    //     	    	new_bank_modes[i] = 1;
    //    	    }
    ...
    ////XJY DRAM END Here
    std::cerr << "new bank mode is below." << std::endl;
    for (int i = 0; i < numberOfBanks; i++)
	{
		setMemBankMode(i, new_bank_modes[i]);
	//	std::cout << new_bank_modes[i] << std::endl;
	}
}
